...
function dayProgressPercent(inputHour, inputMinute) {
    inputHour = Number(inputHour)
    inputMinute = Number(inputMinute)
    totalMinutes = (inputHour * 60) + inputMinute
...
function timeDisplay() {
    ...
    // FROMULA: day progress = (elapsed / duration * 100); elapsed day = (currentHours * 60 + currentMins); day duration = (24 * 60)
    const currentDayProgress = ((((currentHours * 60) + currentMins) / (24 * 60)) * 100).toFixed(2)
...
setInterval(() => {
    // Updates all datime related jobs
    timeDisplay()
}, 1000);